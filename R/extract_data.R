...
extract_data <- function(
  ...
  # Check time step
  temporal.timestep.options = .time_agg_options()$temporal.timestep    # Get list of available time
  ...
  base.agg.fun.outer = .time_agg_options()$temporal.function.outer    # Get list of available functions
  base.agg.fun.inner = .time_agg_options()$temporal.function.inner
  ...
  for (itime.step in vars.extract.summary$time.step) {
    result = tryCatch({
      x = .do_temporal_aggregation(data=NA,
  ...
  # 1. A few grid cells: infill using raster:focal moving average. These are handled within .extract_point_data().
  ...
  nTimestepsPerOutputStep = mapply(.do_temporal_aggregation,
                           data = NA,
  ...
  for (i in 1:nlocations) {
    ...
    data.brick.timaAgg = mapply(.do_temporal_aggregation,
                                data = data.brick,
