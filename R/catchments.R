...
#'
#' The catchments can be used to extract catchment average climate data usng \code{extract_data}
#' @return terra::SpatVector
#' @seealso
#' \code{\link{extract_data}} for extracting catchment average climate data.
#' @export
catchments <- function() {
  path <- system.file("extdata", "catchments.gpkg", package = "BOMcatchr")

  if (path == "") {
    pretty_stop("Internal catchment boundary data file not found. Please reinstall the package.")
  }
