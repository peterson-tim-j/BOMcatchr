...
Often users run \code{build.grids} once to build netCDF data files that contain all variables over the entire record length (which requires ~5GB disk storage) and then use the netCDFs grids for multiple projects, rather than re-building the netCDF for each project. Also, if \code{build.grids} is run with the the netCDF file name pointing to existing files and \code{updateFrom=NA} then the netCDF files will be updated to two days ago.
