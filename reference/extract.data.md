...
extract.data extracts the AWAP climate data for each point or polygon.
For the latter, either the daily spatial mean and variance (or user
defined function) of each climate metric is calculated or the spatial
data is returned.
...
- vars:
  ...
  `rownames(BOMcatchr::grid.summary(ncdfFilename))`.
...
[`build.grids`](https://peterson-tim-j.github.io/BOMcatchr/reference/build.grids.md)
