...
The first step is to create the netCDF file of the gridded data. Here
grids are created for daily precipitation, daily minimum temperature,
daily maximum temperature, daily vapour pressure deficit, daily solar
radiation and only between the dates *update.from* and *update.to*.
Importantly, the file (and those downloaded) are created in the working
directory.

If the latter two dates were not input then data would be downloaded
from 1/1/1900 to two days ago. The netCDF file contains grids of the
...
#> vprp          123      0
#> solarrad      123      0
#> Total run time (DD:HH:MM:SS): 00:00:08:54
```
...
calculated as follows, given includes . Note, because only two months
are being extracted, the default method to handle missing data and
anomalies (e.g. Tmin \> Tmax) during ET calculations is automatically
