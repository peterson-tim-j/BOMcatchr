...
To understand when each variable and time-step was built, and which needs updating, the netCDF data can be examined against the above BoM update schedules - see \code{grid_sources()} for details. The plot below shows the date each time-step of each variable was downloaded. The output from \code{grid_ages} reports if any variables can be updated. Note, the input \code{today} is ONLY used for testing and should not be input during regular use. Here today is set to 12 days into the future.
```{r}
