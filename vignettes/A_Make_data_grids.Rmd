...
Note, in practice users often run \code{grid.build} once to build the netCDF data file that contain all variables over the entire record length (which requires ~5GB disk storage) and then use the netCDF grids for multiple projects, rather than re-building the netCDF grids for each project.
...
```{r}
ncdffile.name <- grid.build(ncdfFilename=ncdfFilename,
...
To understand when each variable and time-step was built, and which needs updating, the netCDF data can be examined against the above BoM update schedules - see \code{grid.sources()} for details. The plot below shows the date each time-step of each variable was downloaded. The output from \code{grid.ages} reports if any variables can be updated. Note, the input \code{today} is ONLY used for testing and should not be input during regular use. Here today is set to 12 days into the future.
```{r}
...
```{r}
ncdffile.name <- grid.build(ncdfFilename=ncdffile.name,
                updateFrom=startDate, updateTo=endDate)
```
