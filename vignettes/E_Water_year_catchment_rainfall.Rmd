...
```
## Calculate seasonal rainfall climatology
...
Additonal advanced measures can also be derived. Here the area weighted seasonal rainfall anomaly is calculated using both the daily and monthly data. To achieve this the input \code{temporal.timestep} is changed to \code{'seasonal'} and the temporal function \code{temporal.fn.inner} is changed. In the first example, the later function is changed to \code{'annom'}. This calculates the seasonal anomaly where the within season data is the mean of the time-step.
