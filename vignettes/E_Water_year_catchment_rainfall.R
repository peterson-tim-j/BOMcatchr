...
for (i in 1:length(site_ID)) {
  ...
  # Water years precipitation from monthly data.
  filt = annom.seaonal$temporal$Location.ID == site_ID[i]
  tmp.date = annom.seaonal$temporal[filt,]
  ...
  tmp.date = cumannom.seaonal$temporal[filt,]
  y.data = tmp.date$precip.monthly
  plot(x = x.data,
       y = y.data,
       type = "l",
       ...
       mgp = c(2, 0.5, 0),
       ylim = c(0, ceiling(max(y.data) * 1.2)),
       ylab = "Precip. cum. annomoly [mm/day]",
       xlab = "Date",
       xaxs = "r",
       bty = "l",
       yaxs = "r",
