...
Multiple more advanced measures can also be derived. To demonstrate, here the area weighted seasonal rainfall anomaly is calculated using both the daily and monthly data. To achieve this only two changes are required. The input \code{temporal.timestep} is changed to \code{'seasonal'} and the temporal function \code{temporal.function.name} is changed. In the first example, the later function is changed to \code{'annom.mean'}. This calculates the seasonal anomaly where the within season data is the mean of the time-step. Many other options are also available such as \code{'annom.sum'}, \code{'annom.min'}, \code{'annom.max'}, \code{'annom.median'}, \code{'annom.var'}.
...
annom.seaonal = extract.data(ncdfFilename = ncdfFilename,
                      ...
                      temporal.function.name = 'annom.mean',
                      spatial.function.name = 'var')
```
...
cumannom.seaonal = extract.data(ncdfFilename = ncdfFilename,
                      ...
                      locations = catch,
                      temporal.timestep = 'seasonal',
                      temporal.function.name = 'cumannom.mean',
                      spatial.function.name = 'var')
```

Now let's compare the two estimates the precipitation anomaly and cumularive anomaly at the two catchments. The blue line shows that calculated from the daily data and the red line shows that calculated from the monthly data.
```{r}
#| fig.width = 8,
#| fig.height = 6
...
for (i in 1:length(catch$CatchID)) {
  ...
  # Water years precipitation from monthly data.
  filt = annom.seaonal$annom.mean$Location.ID == catch$CatchID[i]
  tmp.date = annom.seaonal$annom.mean[filt,]
  ...
  tmp.date = cumannom.seaonal$annom.mean[filt,]
  y.data = tmp.date$precip.monthly
  plot(x = x.data,
       y = y.data,
       type = "l",
