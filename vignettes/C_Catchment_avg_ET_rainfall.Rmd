...
Now that we have the meteorological data we can begin extracting data for two catchments. Here the catchment boundaries built into the package are used. However, shape files of polygons can easily be used once imported into R. Here the data is imported into the _catch_ variable.
```{r}
catch = catchments()
```
...
zlim <- range(values(plot.data), na.rm = TRUE)

for (i in 1:nlyr(plot.data)) {
  plot(plot.data[[i]],
       main = map.labels,
       loc.main = 'topright',
       box=T,
       legend = FALSE)
  plot(catch, add = TRUE, border = "red", lwd = 2)
}

par(mar = c(5, 2, 5, 4))  # adjust margins for legend panel
...
terra::panel(metData.monthly[[colind]],
             main = map.labels,
             loc.main = 'topright',
             box=T,
             plg=list(title="PET (mm/month)"))
```
...
#| fig.width = 8,
#| fig.height = 6
colnames.all = naes(metData.monthly)
...
Now let's compare the two estimates of precipitation, PET and rainfall deficit. The right most plots below show the rainfall deficit. It shows that using centroid estimate introduces errors of up to 6 mm/month error.
```{r}
#| fig.width = 8,
#| fig.height = 6
